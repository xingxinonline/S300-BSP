...
6. 当前人脸检测 DSP 运行期统一只发送一条 `MM_ENABLE` 请求，由 CM4 根据板型决定是否只写 `CORE_REG_UPDATE`，或同时补 `SPI_REG_UPDATE`。
...
| `0x21` | `REQUEST_MASTER_MM_RUNTIME` | 子板请求主板执行一次 MM 运行态使能，具体寄存器写入由主板判定      |
| `0x22` | `REQUEST_MASTER_SPI_SYNC`   | 兼容保留码。当前实现会折算到 `REQUEST_MASTER_MM_RUNTIME` 语义     |
