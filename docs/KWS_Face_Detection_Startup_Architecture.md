...
- v1.4 / 2026-03-12: 收紧子板运行时同步语义，明确 gimbal_node 无本地 LCD，子板 DSP 运行期只请求 `CORE_REG_UPDATE`，`SPI_REG_UPDATE` 保留但当前不用
...
2. **子板 `P4_SUB_MM_STARTING` 必须拆成“子板发请求”和“主板轮询处理”两个动作。**
    因为 OV5640、LCD 以及相关 MM 使能寄存器属于主板外设域，子板 DSP/CM4 只能提出请求，不能直接完成最终使能。
...
4. 子板 CM4 再把这个意图通过板间可轮询寄存器暴露给主板。
5. 最终执行 MM 使能或同步动作的是主板 CM4，而不是子板 DSP，也不是子板 CM4。
6. 当前 gimbal_node 无本地 LCD，因此子板运行期同步只应请求主板侧 `CORE_REG_UPDATE`，不应再发送 `SPI_REG_UPDATE`。

建议语义链如下：

1. `SDSP -> SCM4`：请求主板侧资源使能或同步。
2. `SCM4 -> MCM4`：通过 `REQUEST` 寄存器暴露请求。
3. `MCM4`：轮询到请求后，执行主板侧 MM/显示/分发配置。
4. `MCM4 -> SCM4`：通过 `REQUEST_ACK` 或下一阶段命令表示已处理。
5. `SCM4/SDSP`：收到外部条件满足后，再继续本地状态机。
...
| ------ | -------------------------- | ---------------------------------------------------------------- |
| `0x00` | `NONE`                     | 无请求                                                           |
| `0x20` | `REQUEST_MASTER_MM_ENABLE` | 子板请求主板完成主板侧 MM 使能                                   |
| `0x21` | `REQUEST_MASTER_CORE_SYNC` | 子板请求主板触发主板侧 `CORE_REG_UPDATE`                         |
| `0x22` | `REQUEST_MASTER_SPI_SYNC`  | 保留码。当前 gimbal_node 无本地 LCD，子板运行期不应发送该请求    |
