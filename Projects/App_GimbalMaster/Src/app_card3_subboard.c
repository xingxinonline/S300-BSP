...
#define CARD3_POLL_MS      200u
#define CARD3_RUN_POLL_MS   80u
...
__attribute__((unused)) static const char *detection_type_label(uint8_t raw_type)
{
...
static void log_capabilities(uint8_t capabilities)
{
    ...
    MASTER_LOG_INFO("[MASTER][CARD3] capabilities=0x%02X detection=%u tracking_summary=%u tracking_control=%u gesture=%u\r\n",
...
static void mark_init_success(uint32_t now_ms, uint8_t proto_ver)
{
    ...
    MASTER_LOG_INFO("[MASTER][CARD3] handshake success proto=0x%02X elapsed=%lu ms\r\n",
                    (unsigned)proto_ver,
...
static void mark_init_failed(uint32_t now_ms, const char *reason)
{
    ...
    reset_runtime_state();
    MASTER_LOG_WARN("[MASTER][CARD3] handshake failed: %s elapsed=%lu ms, suspend polling\r\n",
                    reason,
                    (unsigned long)(now_ms - s_init_since_ms));
}
...
{
    return s_ops.read_regs_at(SUBBOARD_STARTUP_SLAVE_ADDR_CARD3, reg, value, 1u);
}

static int read_regs(uint8_t reg, uint8_t *buffer, uint32_t length)
{
    return s_ops.read_regs_at(SUBBOARD_STARTUP_SLAVE_ADDR_CARD3, reg, buffer, length);
}

static int write_reg8(uint8_t reg, uint8_t value)
{
    if (s_ops.write_reg8_at == NULL) {
        return -1;
    }

    return s_ops.write_reg8_at(SUBBOARD_STARTUP_SLAVE_ADDR_CARD3, reg, value);
}
...
static int send_prepare_video_cmd(void)
{
    ...
    return write_reg8(SUBBOARD_STARTUP_REG_CMD, SUBBOARD_STARTUP_CMD_PREPARE_VIDEO);
}
...
static int send_start_dsp_cmd(void)
{
    ...
    return write_reg8(SUBBOARD_STARTUP_REG_CMD, SUBBOARD_STARTUP_CMD_START_DSP);
}
                         ...
                         uint8_t cmd_result)
{
    if (state != s_last_state) {
        MASTER_LOG_INFO("[MASTER][CARD3] subboard_state=%s\r\n", public_state_name(state));
        s_last_state = state;
    }

    if (error != s_last_error) {
        if (error != SUBBOARD_STARTUP_ERR_NONE) {
            MASTER_LOG_WARN("[MASTER][CARD3] subboard_error=0x%02X\r\n", error);
        }
        s_last_error = error;
    }

    if (heartbeat != s_last_heartbeat) {
        MASTER_LOG_DEBUG("[MASTER][CARD3] subboard_heartbeat=%u\r\n", (unsigned)heartbeat);
        s_last_heartbeat = heartbeat;
    }

    if ((request != s_last_request) || (request_ack != s_last_request_ack)) {
        MASTER_LOG_INFO("[MASTER][CARD3] request=%s ack=%s\r\n",
    ...
    if ((cmd_ack != s_last_cmd_ack) || (cmd_result != s_last_cmd_result)) {
        MASTER_LOG_INFO("[MASTER][CARD3] cmd_ack=0x%02X result=%s\r\n",
                        cmd_ack,
    ...
    if (request == SUBBOARD_STARTUP_REQ_MASTER_MM_ENABLE) {
        if (!s_video_prepared) {
            if ((s_ops.prepare_video_path == NULL) || (s_ops.prepare_video_path() != 0)) {
                MASTER_LOG_WARN("[MASTER][CARD3] video path prepare failed, waiting retry\r\n");
                return;
            }
            s_video_prepared = true;
        }
    } else if ((request == SUBBOARD_STARTUP_REQ_MASTER_MM_RUNTIME) ||
               (request == SUBBOARD_STARTUP_REQ_MASTER_SPI_SYNC)) {
        if (s_ops.trigger_mm_runtime_enable != NULL) {
            s_ops.trigger_mm_runtime_enable();
        }
        MASTER_LOG_INFO("[MASTER][CARD3] executed %s\r\n", request_name(request));
    }

    if (write_reg8(SUBBOARD_STARTUP_REG_REQUEST_ACK, request) == 0) {
        MASTER_LOG_INFO("[MASTER][CARD3] acknowledged %s\r\n", request_name(request));
    } else {
        MASTER_LOG_WARN("[MASTER][CARD3] failed to acknowledge %s\r\n", request_name(request));
    }
}
                            ...
                            uint8_t cmd_result)
{
    if (!s_prepare_sent &&
        ...
        if (send_prepare_video_cmd() == 0) {
            MASTER_LOG_INFO("[MASTER][CARD3] sent PREPARE_VIDEO_CONSUMER\r\n");
            s_prepare_sent = true;
        } else {
            MASTER_LOG_WARN("[MASTER][CARD3] failed to send PREPARE_VIDEO_CONSUMER\r\n");
        }
    }
    ...
    if (!s_start_dsp_sent &&
        ...
        if (send_start_dsp_cmd() == 0) {
            MASTER_LOG_INFO("[MASTER][CARD3] sent START_DSP\r\n");
            s_start_dsp_sent = true;
        } else {
            MASTER_LOG_WARN("[MASTER][CARD3] failed to send START_DSP\r\n");
        }
    }
}
...
void app_card3_subboard_tick(void)
{
    ...
    poll_ms = (s_last_state == SUBBOARD_STARTUP_STATE_RUNNING) ? CARD3_RUN_POLL_MS : CARD3_POLL_MS;
    ...
    if (proto_ver != SUBBOARD_STARTUP_PROTO_VER) {
        MASTER_LOG_WARN("[MASTER][CARD3] unsupported proto=0x%02X expected=0x%02X\r\n",
                        (unsigned)proto_ver,
    ...
    if (proto_ver != s_last_proto_ver) {
        MASTER_LOG_INFO("[MASTER][CARD3] subboard online, proto=0x%02X\r\n", proto_ver);
