...
__attribute__((used, noinline)) void fd_state_machine_dsp_image_load_point(void)
{
    __asm volatile("nop");
}
...
int main(void)
{
    ...
    if (SysTick_Config(SystemCoreClock / 1000u) != 0u) {
        printf("[S300][FD-SM][ERR] SysTick_Config failed\r\n");
        while (1) {
            __NOP();
        }
    }
    ...
    init_psram(4, 1);
    fd_state_machine_dsp_image_load_point();
    face_detection_app_init(millis);

    while (1) {
        face_detection_app_tick();
        g_cpu_in_idle = 1;
        {
