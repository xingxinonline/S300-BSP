...
#define HD_BOX_TIMEOUT_MS       500u
#define HD_SCORE_FILTER_PCT     50u
#define HD_ALPHA_SOLID          0xFFu
#define HD_ALPHA_CLEAR          0x00u
#define HD_LOG_EVERY_N_FRAMES   30u
...
static void clear_rect_border(int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
    ...
    for (int32_t x = x1; x <= x2; x++) {
        set_pixel_alpha((uint32_t)x, (uint32_t)y1, HD_ALPHA_CLEAR);
        set_pixel_alpha((uint32_t)x, (uint32_t)y2, HD_ALPHA_CLEAR);
    }

    for (int32_t y = y1 + 1; y < y2; y++) {
        set_pixel_alpha((uint32_t)x1, (uint32_t)y, HD_ALPHA_CLEAR);
        set_pixel_alpha((uint32_t)x2, (uint32_t)y, HD_ALPHA_CLEAR);
    }
}
...
static void process_multi_result(const DetectionResult_t *result)
{
    ...
    if (!is_valid_detection_result_for_demo(result)) {
        if ((result != 0) && !s_proto_warned) {
            printf("[HD-OVL][WARN] drop result: magic=0x%08lX version=0x%04lX count=%lu\r\n",
    ...
    if ((result->version >> 8) == 0x02u) {
        ...
        if (!s_v2_logged) {
            printf("[HD-OVL] compatible protocol mode: DSP version=0x%04lX\r\n",
                   (unsigned long)result->version);
            s_v2_logged = true;
        }
    }
    ...
    for (uint32_t index = 0u; index < result->count && index < MAX_DETECTION_COUNT; index++) {
        ...
        if (score_pct < HD_SCORE_FILTER_PCT) {
            continue;
        }
    ...
    if ((drawn > 0u) &&
        ((s_last_logged_frame_id == 0u) ||
         ((result->frame_id - s_last_logged_frame_id) >= HD_LOG_EVERY_N_FRAMES))) {
        printf("[HD-OVL] frame=%lu humans=%lu\r\n",
...
{
    if ((s_prev_count > 0u) &&
        ((uint32_t)(millis() - s_last_valid_ms) >= HD_BOX_TIMEOUT_MS)) {
        clear_all_boxes();
        printf("[HD-OVL] clear stale boxes\r\n");
    }
}
