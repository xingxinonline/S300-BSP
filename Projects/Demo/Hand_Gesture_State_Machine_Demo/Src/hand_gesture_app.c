...
#include "gpio.h"
#include "hand_gesture_app.h"
#include "hand_gesture_overlay.h"
#include "hand_gesture_runtime_proto.h"
#include "mailbox.h"
...
#define HG_HELLO_RETRY_MS         200u
#define HG_RESOURCE_RETRY_MS      300u
#define HG_HEARTBEAT_INTERVAL_MS  1000u
#define HG_RESPONSE_TIMEOUT_MS    2000u

#define HG_STREAM_ID_MAIN         0x01u
#define HG_CONFIG_SLOT_ID         0x32u
#define HG_BUFFER_SLOT_ID         0x42u
#define HG_REQUIRED_VIDEO_RESOURCES \
    (CONTROL_RESOURCE_CAMERA_READY | CONTROL_RESOURCE_MM_READY | CONTROL_RESOURCE_LCD_READY)

typedef enum {
    HG_STATE_RESET = 0,
    HG_STATE_HANDSHAKING,
    HG_STATE_DSP_READY,
    HG_STATE_CM4_RESOURCE_READY,
    HG_STATE_CONFIGURED,
    HG_STATE_RUNNING,
    HG_STATE_ERROR,
} HandGestureState_t;
...
typedef enum {
    HG_PENDING_NONE = 0,
    HG_PENDING_CONFIG_ACK,
    HG_PENDING_BUFFER_ACK,
    HG_PENDING_START_ACK,
} HandGesturePending_t;

static uint32_t (*s_get_millis)(void) = 0;
static HandGestureState_t s_state = HG_STATE_RESET;
static HandGesturePending_t s_pending = HG_PENDING_NONE;
...
static const char *state_name(HandGestureState_t state)
{
    switch (state) {
    case HG_STATE_RESET: return "RESET";
    case HG_STATE_HANDSHAKING: return "HANDSHAKING";
    case HG_STATE_DSP_READY: return "DSP_READY";
    case HG_STATE_CM4_RESOURCE_READY: return "CM4_RESOURCE_READY";
    case HG_STATE_CONFIGURED: return "CONFIGURED";
    case HG_STATE_RUNNING: return "RUNNING";
    case HG_STATE_ERROR: return "ERROR";
    default: return "UNKNOWN";
    }
}

static void enter_state(HandGestureState_t next_state)
{
    if (s_state != next_state) {
        printf("[HG-SM] STATE %s -> %s\r\n", state_name(s_state), state_name(next_state));
        s_state = next_state;
        s_state_since_ms = millis();
        if (next_state != HG_STATE_RUNNING) {
...
static void trigger_mm_runtime_enable(void)
{
    ...
    printf("[HG-SM] CM4 applied MM runtime enable: core only, local LCD SPI path disabled\r\n");
#else
    trigger_spi_reg_update();
    printf("[HG-SM] CM4 applied MM runtime enable: core + lcd spi\r\n");
#endif
}
...
static int init_video_path(void)
{
    ...
    if (cam_ret != 0) {
        printf("[HG-SM][ERR] OV5640 init failed (%d), video resources not ready\r\n", cam_ret);
        return -1;
    }

    resource_flags |= CONTROL_RESOURCE_CAMERA_READY;

    printf("[HG-SM] init video\r\n");
...
static void control_mailbox_prepare(void)
{
    ...
    printf("[HG-SM] Mailbox re-synced\r\n");
}
...
static void dsp_uart_init(void)
{
    ...
    printf("[HG-SM] UART3 initialized for DSP\r\n");
}

static void log_video_resources_ready(void)
{
    printf("[HG-SM] CM4 video resources ready: camera=%u mm=%u lcd=%u\r\n",
    ...
    printf("[HG-SM] waiting DSP runtime notify for MM runtime enable\r\n");
    printf("[HG-SM] Overlay %ux%u, detection_base=0x%08lX\r\n",
...
static bool handle_runtime_message(uint32_t msg)
{
    ...
    if (!hg_runtime_msg_is_mm_enable_req(msg)) {
        return false;
    }

    if ((s_state != HG_STATE_CONFIGURED) && (s_state != HG_STATE_RUNNING)) {
        printf("[HG-SM][WARN] ignore MM enable req before stream start, state=%s msg=0x%08lX\r\n",
               state_name(s_state),
               (unsigned long)msg);
        return true;
    }

    if ((s_video_resource_flags & HG_REQUIRED_VIDEO_RESOURCES) != HG_REQUIRED_VIDEO_RESOURCES) {
        printf("[HG-SM][WARN] ignore MM enable req=0x%08lX, video resources incomplete\r\n",
               (unsigned long)msg);
        return true;
    }

    enable_req = hg_runtime_msg_get_mm_enable_req(msg);
    if ((enable_req != HG_RT_MM_ENABLE_REQ) &&
        (enable_req != HG_RT_SYNC_REQ_SPI_REG_UPDATE)) {
        printf("[HG-SM][WARN] unknown MM enable payload=0x%08lX\r\n",
...
static void dsp_start_new_session(void)
{
    ...
    printf("[HG-SM] DSP warm reset triggered, session=0x%02X\r\n", s_session_id);
}
...
static void send_control_msg(uint32_t msg, const char *label)
{
    ...
    if (ret == 0) {
        printf("[HG-SM] TX %-20s 0x%08lX\r\n", label, (unsigned long)msg);
    } else {
        printf("[HG-SM] TX %-20s failed (%d)\r\n", label, ret);
    }
}
...
{
    if ((s_video_resource_flags & HG_REQUIRED_VIDEO_RESOURCES) != HG_REQUIRED_VIDEO_RESOURCES) {
        printf("[HG-SM][ERR] Skip SYS.CM4_RESOURCE_READY, CM4 video resources incomplete: flags=0x%02X\r\n",
...
static void send_config_apply(void)
{
    send_control_msg(
        CONTROL_CMD_MAKE(CONTROL_CMD_GRP_CONFIG, s_session_id,
                         CONTROL_CMD_CONFIG_APPLY, HG_CONFIG_SLOT_ID),
        "CMD.CONFIG_APPLY");
    s_pending = HG_PENDING_CONFIG_ACK;
}
...
static void send_buffer_bind(void)
{
    send_control_msg(
        CONTROL_CMD_MAKE(CONTROL_CMD_GRP_BUFFER, s_session_id,
                         CONTROL_CMD_BUFFER_BIND, HG_BUFFER_SLOT_ID),
        "CMD.BUFFER_BIND");
    s_pending = HG_PENDING_BUFFER_ACK;
}

static void send_start_stream(void)
{
    send_control_msg(
        CONTROL_SYS_START_STREAM(s_session_id, HG_STREAM_ID_MAIN, 0x01u),
        "SYS.START_STREAM");
    s_pending = HG_PENDING_START_ACK;
}
...
static void handle_ack(uint32_t msg)
{
    ...
    printf("[HG-SM] RX ACK kind=%u code=0x%02X status=%u\r\n",
           (unsigned)kind, (unsigned)code, (unsigned)status);

    if (kind == CONTROL_RSP_KIND_CMD) {
        if ((s_pending == HG_PENDING_CONFIG_ACK) &&
        ...
        if ((s_pending == HG_PENDING_BUFFER_ACK) &&
            (code == CONTROL_CMD_BUFFER_BIND) &&
            (status == CONTROL_ACK_OK)) {
            s_pending = HG_PENDING_NONE;
            enter_state(HG_STATE_CONFIGURED);
            send_start_stream();
            return;
        }
    }
    ...
    if (kind == CONTROL_RSP_KIND_SYS) {
        if ((s_pending == HG_PENDING_START_ACK) &&
            (code == CONTROL_SYS_SUBTYPE_START_STREAM) &&
            (status == CONTROL_ACK_OK)) {
            s_pending = HG_PENDING_NONE;
            enter_state(HG_STATE_RUNNING);
            printf("[HG-SM] Stream start ACK received, waiting for gesture result mailbox\r\n");
        }
    }
}
...
static void handle_nack(uint32_t msg)
{
    ...
    printf("[HG-SM] RX NACK kind=%u code=0x%02X error=%u\r\n",
           (unsigned)kind, (unsigned)code, (unsigned)error);
    s_pending = HG_PENDING_NONE;
    enter_state(HG_STATE_ERROR);
}

static void handle_status(uint32_t msg)
{
    uint16_t arg = CONTROL_GET_ARG(msg);
    printf("[HG-SM] RX STATUS run_state=%u brief=%u\r\n",
...
static void handle_sys_message(uint32_t msg)
{
    ...
    if (subtype == CONTROL_SYS_SUBTYPE_HELLO_ACK) {
        printf("[HG-SM] RX HELLO_ACK session=0x%02X boot=%u feature=%u state=%u\r\n",
        ...
        if ((s_state == HG_STATE_HANDSHAKING) ||
            (s_state == HG_STATE_RESET)) {
            enter_state(HG_STATE_DSP_READY);
            log_video_resources_ready();
            send_resource_ready();
            enter_state(HG_STATE_CM4_RESOURCE_READY);
        }
        return;
    }

    if (subtype == CONTROL_SYS_SUBTYPE_DSP_MODEL_READY) {
        printf("[HG-SM] RX DSP_MODEL_READY algo=%u flags=%u run_state=%u\r\n",
        ...
        if ((s_state == HG_STATE_CM4_RESOURCE_READY) &&
            (s_pending == HG_PENDING_NONE)) {
            send_config_apply();
        }
        return;
    ...
    if (subtype == CONTROL_SYS_SUBTYPE_HEARTBEAT) {
        printf("[HG-SM] RX DSP HEARTBEAT seq=%u status=%u\r\n",
               (unsigned)CONTROL_HEARTBEAT_GET_SEQ(arg),
               (unsigned)CONTROL_HEARTBEAT_GET_STATUS(arg));
        return;
    }

    printf("[HG-SM] RX SYS subtype=%u arg=0x%04X\r\n",
    ...
    while (1) {
        ...
        case CONTROL_MSG_TYPE_STATUS:
            if (!control_msg_session_matches(msg, s_session_id)) {
                printf("[HG-SM] Drop stale control msg: 0x%08lX (session=0x%02X current=0x%02X)\r\n",
                       (unsigned long)msg,
                       (unsigned)CONTROL_GET_SESSION(msg),
        ...
        default:
            if (s_state == HG_STATE_RUNNING) {
                (void)hand_gesture_overlay_handle_mailbox_message(msg);
            }
            break;
        }
    }
...
static void step_state_machine(void)
{
    ...
    case HG_STATE_RESET:
        ...
        s_pending = HG_PENDING_NONE;
        send_hello();
        enter_state(HG_STATE_HANDSHAKING);
        break;

    case HG_STATE_HANDSHAKING:
        if ((uint32_t)(now_ms - s_last_hello_ms) >= HG_HELLO_RETRY_MS) {
            send_hello();
        }
        break;

    case HG_STATE_CM4_RESOURCE_READY:
        if (((uint32_t)(now_ms - s_last_resource_ms) >= HG_RESOURCE_RETRY_MS) &&
            (s_pending == HG_PENDING_NONE)) {
            send_resource_ready();
        }
        break;

    case HG_STATE_RUNNING:
        if ((uint32_t)(now_ms - s_last_heartbeat_ms) >= HG_HEARTBEAT_INTERVAL_MS) {
            send_heartbeat();
        }
        hand_gesture_overlay_tick();
        break;

    case HG_STATE_DSP_READY:
    case HG_STATE_CONFIGURED:
    case HG_STATE_ERROR:
    default:
        break;
    }

    if (((uint32_t)(now_ms - s_state_since_ms) >= HG_RESPONSE_TIMEOUT_MS) &&
        (s_state != HG_STATE_RUNNING) &&
        (s_state != HG_STATE_RESET)) {
        process_mailbox();
        ...
        if ((uint32_t)(millis() - s_state_since_ms) < HG_RESPONSE_TIMEOUT_MS) {
            return;
        }

        printf("[HG-SM] State timeout in %s, restart session\r\n", state_name(s_state));
        enter_state(HG_STATE_RESET);
    }
}
...
void hand_gesture_app_init(uint32_t (*get_millis_fn)(void))
{
    ...
    if (init_video_path() != 0) {
        enter_state(HG_STATE_ERROR);
        printf("[HG-SM][ERR] Hand gesture demo requires CM4-owned camera/MM/LCD resources\r\n");
        return;
    }

    hand_gesture_overlay_init(get_millis_fn);
    control_mailbox_prepare();
    dsp_uart_init();
    s_state_since_ms = millis();
    printf("[HG-SM] Hand gesture state machine demo started\r\n");
}
