...
static void dump_mm_video_regs(const char *tag)
{
    ...
    SUB_LOG_DEBUG("[SUB-MM] %s disp=%ux%u snap=%ux%u reg20=0x%08lX reg28=0x%08lX\r\n",
                  tag,
...
int subboard_mm_app_start_mm_consumer(SubboardMmAppContext *ctx)
{
    ...
    SUB_LOG_INFO("[SUB-MM] starting MM consumer path\r\n");
    SUB_LOG_INFO("[SUB-MM] OV5640 init skipped; MM/SPI config mirrored from master, LCD SPI output disabled\r\n");

    ret = rcc_init_mm_pll(8, 400, 0, 3, 2);
    if (ret != RCC_STATUS_OK) {
        SUB_LOG_WARN("[SUB-MM] rcc_init_mm_pll failed=%d\r\n", ret);
        return -1;
    }
...
int subboard_mm_app_ensure_dsp_pll_started(SubboardMmAppContext *ctx)
{
    ...
    if (ret != RCC_STATUS_OK) {
        SUB_LOG_WARN("[SUB-MM] rcc_init_dsp_pll failed=%d\r\n", ret);
        return -1;
    }

    ctx->dsp_pll_started = true;
    SUB_LOG_INFO("[SUB-MM] DSP PLL ready\r\n");
    return 0;
}
...
{
    SUB_LOG_INFO("[SUB-MM] CMD START_DSP arg=0x%02X\r\n",
                 subboard_startup_i2c_get_command_arg());

    if (!ctx->mm_started || !ctx->master_mm_granted) {
        SUB_LOG_WARN("[SUB-MM] reject START_DSP: MM path not ready\r\n");
        finish_command(cmd, SUBBOARD_STARTUP_RESULT_BUSY);
        return;
    }

    if (subboard_dsp_ctrl_is_active()) {
        SUB_LOG_WARN("[SUB-MM] reject START_DSP: DSP control already active\r\n");
        finish_command(cmd, SUBBOARD_STARTUP_RESULT_BUSY);
        return;
    }
...
{
    SUB_LOG_INFO("[SUB-MM] CMD CLEAR_ERROR\r\n");
    subboard_startup_i2c_set_error_code(SUBBOARD_STARTUP_ERR_NONE);
...
{
    SUB_LOG_WARN("[SUB-MM] CMD 0x%02X not supported\r\n", cmd);
