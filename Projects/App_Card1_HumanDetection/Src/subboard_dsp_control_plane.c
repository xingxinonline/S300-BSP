                                                       ...
                                                       SubboardDspControlEffect_t *out_effect)
{
    ...
    if ((state != SUB_DSP_STATE_CONFIGURED) && (state != SUB_DSP_STATE_RUNNING)) {
        SUB_LOG_WARN("[CARD1-DSP] ignore MM enable req before stream start, state=%s msg=0x%08lX\r\n",
    ...
    if ((enable_req != SUBBOARD_RT_MM_ENABLE_REQ) &&
        ...
        SUB_LOG_WARN("[CARD1-DSP] unknown MM enable payload=0x%08lX\r\n",
                     (unsigned long)enable_req);
        return true;
    }

    out_effect->request_master_mm_runtime = true;
    SUB_LOG_INFO("[CARD1-DSP] DSP requested MM runtime enable\r\n");
    return true;
}
                                                       ...
                                                       SubboardDspControlEffect_t *out_effect)
{
    ...
    if (!control_msg_session_matches(msg, session_id)) {
        SUB_LOG_WARN("[CARD1-DSP] drop stale control msg: 0x%08lX\r\n", (unsigned long)msg);
        return;
    }
    ...
    if (type == CONTROL_MSG_TYPE_SYS) {
        ...
        if (subtype == CONTROL_SYS_SUBTYPE_HELLO_ACK) {
            SUB_LOG_INFO("[CARD1-DSP] RX HELLO_ACK session=0x%02X boot=%u feature=%u state=%u\r\n",
        ...
        if (subtype == CONTROL_SYS_SUBTYPE_DSP_MODEL_READY) {
            SUB_LOG_INFO("[CARD1-DSP] RX DSP_MODEL_READY algo=%u flags=%u run_state=%u\r\n",
        ...
        if (subtype == CONTROL_SYS_SUBTYPE_HEARTBEAT) {
            ...
            if (status != CONTROL_RUN_STATE_RUNNING) {
                SUB_LOG_DEBUG("[CARD1-DSP] RX DSP HEARTBEAT seq=%u status=%u\r\n",
        ...
        SUB_LOG_DEBUG("[CARD1-DSP] RX SYS subtype=%u arg=0x%04X\r\n",
    ...
    if (type == CONTROL_MSG_TYPE_ACK) {
        ...
        SUB_LOG_DEBUG("[CARD1-DSP] RX ACK kind=%u code=0x%02X status=%u\r\n",
    ...
    if (type == CONTROL_MSG_TYPE_NACK) {
        ...
        SUB_LOG_WARN("[CARD1-DSP] RX NACK kind=%u code=0x%02X error=%u\r\n",
    ...
    if (type == CONTROL_MSG_TYPE_STATUS) {
        ...
        if ((run_state != CONTROL_RUN_STATE_RUNNING) || (brief == 0u)) {
            SUB_LOG_DEBUG("[CARD1-DSP] RX STATUS run_state=%u brief=%u\r\n",
                          (unsigned)run_state,
                          (unsigned)brief);
        }
    }
